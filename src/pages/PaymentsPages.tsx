            ...
            <div className="p-5 space-y-4">
              ...
              <SearchableSelect
                ...
                options={store.customers.filter(c => c.is_active).map(c => ({
                  ...
                  sublabel: `Mobile: ${c.mobile} • Outstanding: ${formatCurrency(store.getCustomerOutstanding(c.id))}`
                }))}
              ...
              {form.payment_mode !== 'Cash' && (
                <SearchableSelect
                  ...
                  options={store.cashBankAccounts.filter(acc => acc.is_active && acc.account_type === 'BANK').map(acc => ({
                    value: acc.id,
                    label: acc.account_name,
                    sublabel: `${acc.bank_name || ''} • Balance: ${formatCurrency(acc.balance)}`
                  }))}
            ...
            <div className="p-5 space-y-4">
              ...
              <SearchableSelect
                ...
                options={store.suppliers.filter(s => s.is_active).map(s => ({
                  ...
                  sublabel: `Mobile: ${s.mobile} • Outstanding: ${formatCurrency(store.getSupplierOutstanding(s.id))}`
                }))}
